import Layout from "~/components/layout/Layout";

export default function GetInTouchPage() {
  return (
    <Layout
      title="Get in touch"
      description="Don't hesitate to get involved and start following. Get in touch if you want to share a social platform project, apply to join directory or simply share a thought."
    >
      <section class="pt-xl pb-2xl">
        <header class="mb-xl">
          <h1 class="font-normal text-4xl md:text-5xl lg:text-7xl">Get in touch</h1>
        </header>
      </section>

      <section class="pt-4xl pb-0 lg:pb-6xl">
        <div class="max-w-[1400px] mx-auto">
          <div class="flex flex-col gap-y-xl">
            <div class="flex flex-row items-center space-x-lg lg:space-x-2xl">
              <div class="w-8 h-8 lg:w-20 lg:h-20 rounded-full bg-dark" />
              <a href="mailto:team@rebuild.net" class="text-xl lg:text-8xl hover:underline">
                Email us
              </a>
            </div>
            <div class="flex flex-row items-center space-x-lg lg:space-x-2xl">
              <div class="w-8 h-8 lg:w-20 lg:h-20 rounded-full bg-dark" />
              <a href="#" data-form="builder-application" class="text-xl lg:text-8xl hover:underline">
                Join the directory
              </a>
            </div>
            <div class="flex flex-row items-center space-x-lg lg:space-x-2xl">
              <div class="w-8 h-8 lg:w-20 lg:h-20 rounded-full bg-dark" />
              <a href="#" data-form="builder-promo" class="text-xl lg:text-8xl hover:underline">
                Suggest a platform
              </a>
            </div>
            <div class="flex flex-row items-center space-x-lg lg:space-x-2xl">
              <div class="w-8 h-8 lg:w-20 lg:h-20 rounded-full bg-dark" />
              <a
                href="https://linkedin.com/company/rebuild-europe"
                class="text-xl lg:text-8xl hover:underline"
              >
                Follow us on Linkedin
              </a>
            </div>
            <div class="flex flex-row items-center space-x-lg lg:space-x-2xl">
              <div class="w-8 h-8 lg:w-20 lg:h-20 rounded-full bg-dark" />
              <a href="#" data-form="newsletter" class="text-xl lg:text-8xl hover:underline">
                Get the newsletter
              </a>
            </div>
          </div>

          <div class="mt-4xl pb-6xl">
            <h2 class="text-2xl">
              <a class="underline" href="/downloads/rebuild-press-kit-dec25.zip">
                Download press kit
              </a>
            </h2>
          </div>
        </div>
      </section>
    </Layout>
  );
}
